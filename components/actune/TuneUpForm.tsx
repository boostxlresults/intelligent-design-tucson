...
// In-area heuristic: Tucson metro ZIPs start 856/857. Soft check only — never a hard block.
function inServiceArea(zip: string): boolean {
  const z = zip.replace(/\D/g, "");
  return z.length < 5 || z.startsWith("856") || z.startsWith("857");
}

export default function TuneUpForm() {
  ...
  const onFirstFocus = () => {
    ...
    dl({ event: "form_start", form_name: "ac_tune_up_2888" });
    fbq("trackCustom", "FormStart", { form: "ac_tune_up_2888" });
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Conversion events fire on the confirmation URL (per spec). Navigate there now.
      window.location.href = "/ac-tune-up-2888/confirmed";
    } catch {
      setStatus("error");
    }
  };
  ...
  return (
    <form onSubmit={submit} className="grid gap-3" data-testid="ac-tune-up-form" noValidate>
      ...
      <button type="submit" disabled={status === "sending"}
        ...
        data-testid="submit-tune-up">
        {status === "sending" ? "Booking…" : "Book Your $28.88 AC Maintenance Special Online"}
      </button>
