...
### Frontend
- **Framework:** Next.js 15.5.12 with App Router, TypeScript, React 19.2.x, and React Server Components.
...
- **RealWorkLabs SEO Indexability:** Location pages with RealWorkLabs maps include additional JSON-LD schemas (ItemList of CreativeWork projects, per-service-type Service schemas with Reviews and AggregateRating) plus visible review cards below the widget. This makes the JS-rendered widget content indexable by Google. Data sourced from `data/projectReviews.ts`, schemas generated by `lib/seo/generateProjectShowcaseSchema.ts`.
